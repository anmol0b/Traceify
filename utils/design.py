...
def render_profile(profile: dict[str, Any]) -> None:
    ...
    if bio:
        ...
        st.markdown('<div class="profile-bio" style="opacity:0.3;font-style:italic">No bio available.</div>', unsafe_allow_html=True)

    st.markdown(f"""
    <div class="profile-meta">
        {"<span>📍 <strong>" + profile['location'] + "</strong></span>" if profile.get('location') else ""}
        {"<span>🔗 <strong>" + profile['website'] + "</strong></span>" if profile.get('website') else ""}
        {"<span>📅 <strong>" + profile['joined_at'] + "</strong></span>" if profile.get('joined_at') else ""}
    </div>
    """, unsafe_allow_html=True)
